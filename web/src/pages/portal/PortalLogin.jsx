...
export default function PortalLogin() {
  const { business, loading, login } = usePortalAuth();
  ...
  if (!loading && business) {
    return <Navigate to={location.state?.from?.pathname || '/portal'} replace />;
  }
    ...
    <div className="ad" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div className="ad-card" style={{ padding: 28 }}>
          ...
          <h1 className="ad-heading" style={{ fontSize: 24, fontWeight: 700, margin: '0 0 8px', letterSpacing: '-0.02em' }}>
            Hyni në llogarinë tuaj
          </h1>
          <p style={{ fontSize: 13, color: '#8FA0B2', margin: '0 0 22px', lineHeight: 1.5 }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 18 }}>
          <Link to="/" className="ad-hint" style={{ textDecoration: 'underline' }}>← Kthehu në faqe</Link>
        </div>
      </div>
    </div>
  );
